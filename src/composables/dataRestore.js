...
export function useDataRestore() {
  ...
  function resetAverages() {
    Object.keys(averageDataRow2Decision).map((el) => {
      if (el !== "decision") {
        averageDataRow2Decision[el] = 0;
      }
    });
    Object.keys(averageDataRow4Decision).map((el) => {
      if (el !== "decision") {
        averageDataRow4Decision[el] = 0;
      }
    });
      ...
      if (tableItem.decision !== 0) {
        if (tableItem.decision == 2) {
          Object.entries(averageDataRow2Decision).forEach((averageValue) => {
            averageDataRow2Decision[averageValue[0]] += isNaN(
              tableItem[averageValue[0]]
            )
              ? 1
              : parseInt(tableItem[averageValue[0]]);
          });
          countedRowLength2Decision.value++;
        } else if (tableItem.decision == 4) {
          Object.entries(averageDataRow4Decision).forEach((averageValue) => {
            averageDataRow4Decision[averageValue[0]] += isNaN(
              tableItem[averageValue[0]]
            )
              ? 1
              : parseInt(tableItem[averageValue[0]]);
      ...
      Object.entries(element).forEach((el) => {
        if (el[0] !== "id" && el[0] !== "decision") {
          ...
          if (element.decision == 2) {
            element[el[0]] = Math.ceil(
              averageDataRow2Decision[el[0]] / countedRowLength2Decision.value
            );
          } else if (element.decision == 4) {
            element[el[0]] = Math.ceil(
              averageDataRow4Decision[el[0]] / countedRowLength4Decision.value
            );
          }
  ...
  return { restoreData, getDeletedRows: store.getDeletedRows };
}
